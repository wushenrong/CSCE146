...
public class VideoGamesDatabaseManager {
  ...
  /**
   * Print out the search result, if result was not printed, prompt the user to search for a game
   * and return to main menu. Else the results were printed and ask the user if they wants to save
   * the results to a file. Inputs are verified by using a while true loops and if the user say no,
   * return to main menu, if the user says yes, call writeVideoGamesSearchResult before returning to
   * the main menu. Else prompt the user for a valid input.
   */
  ...
  /**
   * Write search results to a file. Prompt the user for the filename and ask if they want to append
   * or overwrite the file using the same input validation check as the printVideoGamesResult. Then
   * write the results to the file. If the operation was successful, print out that the operation
