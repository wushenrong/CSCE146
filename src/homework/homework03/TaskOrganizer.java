...
public class TaskOrganizer {
    ...
    /**
     * Adds a task to the task list and organizes it by its priority and when it
     * was added to the list. If the same task with the same priority was added,
    ...
    public void addTask(Task data) {
        ...
        if (organizedTasks[taskPriority].contains(data)) {
            System.out.println(
                    "Task \"" + data.getAction() + "\" with priority " + data.getPriority() + " already exists");
            System.out.println("Skipping Task \"" + data.getAction() + "\" with priority " + data.getPriority());
            return;
        }
    ...
    /**
     * Removes a task by getting the task list based on the task's priority,
     * then manually looping through the task list to remove the task. If the
     * current task is equal to the task given, remove it and return. Else move
     * to the next task until there are no task to check.
     */
    ...
    /**
     * Print each task based on their priority from 0 to 4. Loops over the
     * organized tasks array by priority and then looping over the task linked
     * list manually before printing out the task action based on when the task
