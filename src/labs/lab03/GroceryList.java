...
public class GroceryList {
  ...
  /**
   * Adds a new grocery item to the linked list by checking if the grocery item to add exists. Then
   * check if the list is empty with a blank node, if it is then add the grocery item to the empty
   * node, else find the last node in the list from the head of the list and then create and link a
   * new node with the item to the node at the end of the list.
   */
   ...
   * the reference of the head node to the next node before removing the current node. If the
   * current node is null then do nothing.
   */
